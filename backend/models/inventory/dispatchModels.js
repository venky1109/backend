...
export const DispatchOrder = {
  async findAll(limit = 100, offset = 0) {
    const { rows } = await query(
      ...
      LEFT JOIN dispatch.dispatch_order_items i
        ON i.dispatch_order_id = d.id

      LEFT JOIN catalog.product_barcodes pb
        ON pb.id = i.product_barcode_id
...
export const DispatchOrderItem = {
  async findByDispatchOrderId(dispatchOrderId) {
    const { rows } = await query(
      `
      SELECT 
        i.*,
      ...
      FROM dispatch.dispatch_order_items i
      LEFT JOIN catalog.product_barcodes pb
        ON pb.id = i.product_barcode_id
  ...
  async create(data) {
    ...
    const { rows } = await query(
      ...
      `,
      [
        Number(dispatch_order_id),
        product_barcode_id ? Number(product_barcode_id) : null,
        Number(product_id),
  ...
  async removeByDispatchOrderId(dispatchOrderId) {
    const { rows } = await query(
      `
      DELETE FROM dispatch.dispatch_order_items
      WHERE dispatch_order_id = $1
